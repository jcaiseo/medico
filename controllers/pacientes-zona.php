 ...
 <table id="table" class="tabla display" cellspacing="0" width="1420px">
    <thead style="top: 0; display: block; position: sticky;">
   	<tr>
        ...
        <th width="60px">MOVIL</th>
        <th width="60px">HORA DESP.</th>
...
<?php
	...
	$query = mysql_query("SELECT ag_medico_paciente.id AS id_registro,
                        ...
                        ag_medico_paciente.id_user AS user_a,
                        (SELECT usuarios.user FROM usuarios WHERE usuarios.id_user = user_a) AS user,
						...
						DATE_FORMAT(ag_medico_paciente.hora_despacho, '%H:%i') AS hora_despacho,						
						...
						(SELECT CASE WHEN hora_despacho !='0000-00-00 00:00:00' && movil!='' THEN 'primary'
						END) AS proceso,
						ag_medico_paciente.zona AS zona
						FROM ag_medico_paciente, personas
						WHERE ag_medico_paciente.rut = personas.rut
						AND ag_medico_paciente.estado = '0'
						AND ag_medico_paciente.zona = '$zona'
    ...
    if(mysql_num_rows($query) > 0){
		...
		<tr>
			...
			<td width="100px"><?php echo $row['persona']; ?></td>
			...
			<td width="60px"><?php echo $row['movil']; ?></td>
            <td width="60px"><?php echo $row['hora_despacho']; ?></td>
