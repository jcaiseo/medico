...
if(isset($_POST['guardar'])){
  ...
  if(!isset($error)){	 					 
	 ...
	 $query = mysql_query("INSERT INTO personas(rut, vrut, nombres, apellidopaterno, apellidomaterno, nombre_calle, numdirec, referenciadir, nombre_comuna, fono, fono_2, email, fecha_nacimiento, sexo, unidad_v, fecha_reg) 
						  VALUES ('$rut','$digito', '$nombres', '$apellido_paterno', '$apellido_materno',
	                      '$direccion', '$numero','$referencia_direc', '$nombre_comuna', '$telefono', 
                        '$telefono2', '$email','$fecha_nac', '$sexo','$uv', NOW())")  or die(mysql_error());
					 
	  if($query) { //si esta ok se registra al nuevo usuario
					echo "<script>window.location.href = 'inscribir-paciente.php?id=$id&rut=".$rut_full."';</script>";   
...
<div class="breadcrum">
    <a href="consult.php">Inicio</a> > Identificación de Vecino > Registro de Vecino
</div>    
  ...
  $("#fecha_nac").change(function() {
    ...
    <div class="form-group">
        ...
        <div class="formulario pull-center">
    	        ...
    	        <button type="button" class="btn btn-primary" onclick="javascript:document.location.href = 'consult.php'"><span class="glyphicon glyphicon-repeat"></span> Volver</button>
        </div>
